          ...
          >
            <header className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center space-x-3">
                <Settings className="text-apti-primary" size={24} />
            ...
            <div className="p-6 space-y-8">
                ...
                <div className="flex items-center justify-between p-4 bg-slate-100 dark:bg-slate-800 rounded-xl">
                  ...
                  <button
                    ...
                    className={`relative w-14 h-7 rounded-full transition-colors ${
                      settings.soundEnabled ? 'bg-apti-primary' : 'bg-slate-600'
                    }`}
                    role="switch"
                  ...
                  {themeOptions.map((option) => (
                    <button
                      ...
                      className={`flex flex-col items-center p-4 rounded-xl transition-all border-2 ${
                        settings.theme === option.value
                          ? 'border-apti-primary bg-apti-primary/10'
                    ...
                    >
                      <option.icon
                        size={20}
                        className={`mb-2 ${
                          settings.theme === option.value ? 'text-apti-primary' : 'text-slate-400'
                        }`}
                      />
                      <span
              ...
              <section aria-labelledby="stats-section">
                ...
                <div className="grid grid-cols-2 gap-4 p-4 bg-slate-100 dark:bg-slate-800 rounded-xl">
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-bold text-apti-primary">{stats.totalTestsCompleted}</div>
                    <div className="text-xs text-slate-500 uppercase tracking-wider">Tests</div>
                  </div>
                  <div className="text-center">
                    <div className="text-2xl font-bold text-apti-secondary">{stats.totalQuestionsAnswered}</div>
            ...
            <footer className="p-6 border-t border-slate-200 dark:border-slate-800 text-center">
              <p className="text-xs text-slate-500">AptiCalc v1.0.0 - Built with React + Vite</p>
            </footer>
          </motion.div>
        </motion.div>
      )}
